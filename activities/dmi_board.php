<?php
    ...
    if (isset($_POST["mode"]) && $_POST["mode"] === "delete") {
        ...
        $sql = "delete from activities where num = $num";
     ...
     //   include_once $_SERVER['DOCUMENT_ROOT'] . "/myproject_homepage/db/db_connect.php";
      ...
      history.go(-1)
        ...
        $sql = "insert into activities (id, name, subject, content, regist_day, hit,  file_name, file_type, file_copied) ";
   ...
   ";
            ...
            if (isset($_FILES["upfile"])) {
                ...
                if ($upfile_name && !$upfile_error) { // 업로드가 잘되었는지 판단
                    ...
                    if ($upfile_size > 2000000) {
                        echo("
                                <script>
                                alert('업로드 파일 크기가 지정된 용량(2MB)을 초과합니다! ');
                                history.go(-1)
                                </script>
                                ");
                        exit;
        ...
        $sql = "update activities set subject='$subject', content='$content',  file_name='$upfile_name', file_type='$upfile_type', file_copied= '$copied_file_name'";
        $sql .= " where num=$num";
        mysqli_query($con, $sql);
